...
class HastiXbar(nMasters: Int, addressMap: Seq[UInt=>Bool])(implicit p: Parameters) extends HastiModule()(p) {
  ...
  val lockedM = Reg(init = VecInit(Seq.fill(nMasters)(Bool(false))))
  ...
  val unionGrantMS = VecInit.tabulate(nMasters) { m => VecInit.tabulate(nSlaves) { s =>
  ...
  val bubbleM =
    VecInit.tabulate(nMasters) { m =>
      VecInit.tabulate(nSlaves) { s => dataPhaseGrantSM(s)(m) && !matchMS(m)(s) }
      .reduce(_ || _) }
  ...
  val requestSM = VecInit.tabulate(nSlaves) { s => VecInit.tabulate(nMasters) { m =>
...
class HastiTestSRAM(depth: Int)(implicit p: Parameters) extends HastiModule()(p) {
  ...
  val ready = chisel3.util.random.LFSR(16)(Bool(true))(0) // Bool(true)
  
  // Calculate the bitmask of which bytes are being accessed
  val mask_decode = VecInit.tabulate(hastiAlignment+1) (UInt(_) <= io.hsize)
  val mask_wide   = VecInit.tabulate(hastiDataBytes) { i => mask_decode(log2Ceil(i+1)) }
  val mask_shift  = if (hastiAlignment == 0) 1.U else
  ...
  if (hastiAlignment >= 1) {
    assert (io.htrans === HTRANS_IDLE || io.htrans === HTRANS_BUSY ||
      (io.haddr & mask_decode.asUInt()(hastiAlignment,1)) === 0.U,
  ...
  val d_wdata = VecInit.tabulate(hastiDataBytes) { i => io.hwdata(8*(i+1)-1, 8*i) }
