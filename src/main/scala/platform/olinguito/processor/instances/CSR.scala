...
class MIP(implicit p: Parameters) extends OlinguitoBundle()(p)
...
class PTBR(implicit p: Parameters) extends OlinguitoBundle()(p) {
...
class PerfCounterIO(implicit p: Parameters) extends OlinguitoBundle
    with HasOlinguitoParameters {
  val eventSel = UInt(OUTPUT, xLen)
  val inc = UInt(INPUT, log2Ceil(1+retireWidth))
}

class CSRFileIO(implicit p: Parameters) extends OlinguitoBundle
    with HasOlinguitoParameters {
...
class CSRFile(perfEventSets: EventSets = new EventSets(Seq()))(implicit p: Parameters) extends OlinguitoModule()(p)
    with HasOlinguitoParameters {
  val io = new CSRFileIO

  val reset_mstatus = Wire(init=0.asTypeOf(new MStatus()))
  ...
  val reset_dcsr = Wire(init=0.asTypeOf(new DCSR()))
  ...
  io.interrupts.seip.foreach { mip.seip := reg_mip.seip || RegNext(_) } // CKDUR: This only works as a "if" for the VM mode
  mip.rocc := io.rocc_interrupt
  val read_mip = mip.asUInt & supported_interrupts

  val pending_interrupts = read_mip & reg_mie
  val m_interrupts = Mux(reg_mstatus.prv <= PRV.S || (reg_mstatus.prv === PRV.M && reg_mstatus.mie), pending_interrupts & ~reg_mideleg, 0.U)
  val s_interrupts = Mux(m_interrupts === 0 && (reg_mstatus.prv < PRV.S || (reg_mstatus.prv === PRV.S && reg_mstatus.sie)), pending_interrupts & reg_mideleg, 0.U)
  val (anyInterrupt, whichInterrupt) = chooseInterrupt(Seq(s_interrupts, m_interrupts))
  val interruptMSB = BigInt(1) << (xLen-1)
  val interruptCause = interruptMSB.U + whichInterrupt
  ...
  when (Bool(usingDebug) && reg_dcsr.debugint && !reg_debug) {
    io.interrupt := true
    io.interrupt_cause := interruptMSB.U + CSR.debugIntCause
  }
  ...
  val reg_misa = Reg(init=isaMax.U)
  ...
  val debugTVec = Mux(reg_debug, Mux(insn_break, 0x800.U, 0x808.U), 0x800.U)
  ...
  when (io.rw.cmd.isOneOf(CSR.S, CSR.C, CSR.W)) {
    ...
    if (usingVM) {
      ...
      when (decoded_addr(CSRs.sip)) {
        val new_sip = wdata.asTypeOf(new MIP())
        reg_mip.ssip := new_sip.ssip
      }
      when (decoded_addr(CSRs.sptbr)) {
        val new_sptbr = wdata.asTypeOf(new PTBR())
    ...
    if (nBreakpoints > 0) {
      ...
      when (!bp.control.dmode || reg_debug) {
        when (decoded_addr(CSRs.tdata1)) {
          val newBPC = wdata.asTypeOf(new BPControl())
    ...
    if (reg_pmp.nonEmpty) for (((pmp, next), i) <- (reg_pmp zip (reg_pmp.tail :+ reg_pmp.last)) zipWithIndex) {
      ...
      when (decoded_addr(CSRs.pmpcfg0 + pmpCfgIndex(i)) && !pmp.cfgLocked) {
        pmp.cfg := (wdata >> ((i * pmp.cfg.getWidth) % xLen)).asTypeOf(new PMPConfig())
      }
  ...
  def formEPC(x: UInt) = ~(~x | Cat(!reg_misa('c'-'a'), 1.U))
