...
class HastiRAM(val depth: Int)(implicit p: Parameters) extends Module {
...
class HastiSRAM(val depth: Int)(implicit p: Parameters) extends Module {
  val io = IO(new HastiSlaveIO()(p.alterPartial({
    case HastiId => "00000"
    case HastiKey("00000") =>
      HastiParameters(
  ...
  val ram = SeqMem(depth-Alignment, Vec(DataBits/8, Bits(width = 8)))
